...
impl ChaCha20 {
    ...
    unsafe fn encrypt_bytes(&mut self, mut m: *const u8, mut c: *mut u8, mut bytes: u64) {
        ...
        let      j0 = Wrapping(self.state[ 0]);
        let      j1 = Wrapping(self.state[ 1]);
        let      j2 = Wrapping(self.state[ 2]);
        let      j3 = Wrapping(self.state[ 3]);
        let      j4 = Wrapping(self.state[ 4]);
        let      j5 = Wrapping(self.state[ 5]);
        let      j6 = Wrapping(self.state[ 6]);
        let      j7 = Wrapping(self.state[ 7]);
        let      j8 = Wrapping(self.state[ 8]);
        let      j9 = Wrapping(self.state[ 9]);
        let     j10 = Wrapping(self.state[10]);
        let     j11 = Wrapping(self.state[11]);
        let mut j12 = Wrapping(self.state[12]);
        let mut j13 = Wrapping(self.state[13]);
        let     j14 = Wrapping(self.state[14]);
        let     j15 = Wrapping(self.state[15]);
