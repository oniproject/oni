use std::cmp;
use std::ops::{Add, Sub, BitXor, Shr, Shl};
use byteorder::{LE, ByteOrder};

/*
use buffer::{BufferResult, RefReadBuffer, RefWriteBuffer};
use symmetriccipher::{Encryptor, Decryptor, SynchronousStreamCipher, SymmetricCipherError};
use cryptoutil::{LE::read_u32, symm_enc_or_dec, write_u32_le, xor_keystream};
use simd::u32x4;
*/
...
impl Add for u32x4 {
    ...
    fn add(self, other: Self) -> Self {
        u32x4(self.0.wrapping_add(other.0),
...
impl Sub for u32x4 {
    ...
    fn sub(self, other: Self) -> Self {
        u32x4(self.0.wrapping_sub(other.0),
...
macro swizzle($b: expr, $c: expr, $d: expr) {{
...
macro state_to_buffer($state: expr, $output: expr) {{
    ...
    for i in 0..lens.len() {
        LE::write_u32(&mut $output[i*4..(i+1)*4], lens[i]);
    }
}}
...
macro rotate($a: expr, $b: expr, $c:expr) {{
    let v = $a ^ $b;
    $a = (v << $c) ^ (v >> (S32 - $c))
}}
...
impl ChaCha20 {
    ...
    fn expand(key: &[u8], nonce: &[u8]) -> ChaChaState {
        ...
        ChaChaState {
            a: u32x4(
                LE::read_u32(&constant[0..4]),
                LE::read_u32(&constant[4..8]),
                LE::read_u32(&constant[8..12]),
                LE::read_u32(&constant[12..16])
            ),
            b: u32x4(
                LE::read_u32(&key[0..4]),
                LE::read_u32(&key[4..8]),
    ...
    fn hchacha20(&mut self, out: &mut [u8]) -> () {
        ...
        for i in 0..lens.len() {
            LE::write_u32(&mut out[i*4..(i+1)*4], lens[i]);
        }
    }
